"""
MarketMind — Database Models (updated for scoring engine v2.1)
================================================================
Changes from original:
  SignalsCache  — added 11 new columns (score audit + backtest + coverage)
  AIInsights    — added 8 new columns (consensus + backtest snapshot)

All new columns are nullable so existing rows are unaffected.
Run migrations/add_score_v2_1_columns.py to ALTER the live tables.
"""
...
from sqlalchemy import (
    ...
    Numeric, BigInteger, JSON, Enum, SmallInteger, UniqueConstraint
)
...
class PriceHistory(Base):
    ...
    no_of_trades = Column(Integer)   # used for trades_shock in TechnicalData
    source       = Column(Enum(
        "bhavcopy", "yfinance_fallback", "eod_computed", "historical_import"
    ))
...
class SignalsCache(Base):
    """
    Per-symbol signals cache.
    v2.0 columns: composite_score through momentum_breakdown (existing)
    v2.1 columns: score_version onwards (new — require migration)
    """
    ...
    # ── Scoring engine v2.0 columns (already in schema) ──────────────
    ...
    # ── Scoring engine v2.1 columns (NEW — run migration first) ──────
    # Audit trail
    score_version   = Column(String(10),  nullable=True,
                             comment="Engine version that produced this row")
    scored_at       = Column(DateTime,    nullable=True,
                             comment="UTC timestamp of scoring run")

    # Per-component data coverage (0–1) — supports data_confidence display
    fa_coverage        = Column(Numeric(4, 3), nullable=True,
                                comment="FA fields present / total FA fields")
    ta_coverage        = Column(Numeric(4, 3), nullable=True,
                                comment="TA fields present / total TA fields")
    momentum_coverage  = Column(Numeric(4, 3), nullable=True,
                                comment="Momentum fields present / total")

    # Sector ranking quality
    sector_peer_count  = Column(SmallInteger, nullable=True,
                                comment="Number of sector peers used for relative rank")

    # Backtest results (populated by background job, not per-request)
...
class FundamentalsCache(Base):
    ...
    data_quality       = Column(Enum(
        "FULL", "PARTIAL", "MISSING", "AI_RESEARCHED", "VERIFIED"
    ), default="FULL")
...
class AIInsights(Base):
    """
    AI-generated analysis per symbol per skill.
    v2.0 fields: skill_id, verdict, sentiment_score (existing)
    v2.1 fields: consensus + backtest snapshot (new — run migration first)
    """
    ...
    # ── v2.1 consensus fields (NEW — run migration first) ─────────────
    consensus_score   = Column(Numeric(4, 1), nullable=True,
                               comment="Weighted multi-skill consensus -10 to +10")
    ...
    forensic_veto     = Column(Boolean, default=False, nullable=True,
                               comment="True if SEBI/Hindenburg returned AVOID/CRITICAL")
    all_verdicts      = Column(JSON, nullable=True,
                               comment="Dict of skill_id → verdict string")

    # Score context at time of generation (for drift tracking over time)
...
class AICallLog(Base):
    ...
    id                = Column(Integer, primary_key=True, index=True, autoincrement=True)
    insight_id        = Column(Integer, nullable=True)
    symbol            = Column(String(20), index=True, nullable=False)
    skill_id          = Column(String(50))
